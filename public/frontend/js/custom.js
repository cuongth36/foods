$(document).ready(function() {
    ...
    $('.category-show').click(function(e) {
        ...
        // Check height category product
        var getHeightCateProd = parent.find('.list-category.show').height();
        console.log(getHeightCateProd);
        if (parseInt(getHeightCateProd) >= 240) {
            parent.find('.list-category.show').css("overflow-y", "scroll");
        }
    });
    ...
    $('.submit-fillter-price input').click(function(e) {
        ...
        var link = parent.find('.current-url').data('url');
        var item = new URL(link);
        var orderBy = item.searchParams.get('order_by');
        let url = $('.action-fillter-price', parent).data('action');
        let request = $('.action-request-url', parent).data('request');
        let data = { min: min, max: max, _token: token, request: request, orderBy: orderBy };
        $('.loader-wrapper').show();
        $.ajax({
            url: url,
            method: 'GET',
    ...
    $(document).on("click", ".product-fillter-pagination .page-numbers li", function(e) {
        e.preventDefault();
        var current = $(this);
        var link = current.find('.item').attr('href');
        var element = new URL(link);
        var page = element.searchParams.get("page");
        var min = element.searchParams.get("min");
        var max = element.searchParams.get("max");
        var url = '../danh-muc/san-pham/pagination';
        var request = current.closest('.page-numbers').find('.request-filler-action').data('action');
        var token = $('meta[name="csrf-token"]').attr('content');
        var link = current.closest('.page-numbers').find('.item').attr('href');
        var item = new URL(link);
        var orderBy = item.searchParams.get('order_by');
        var urlRedirect = url + '?page=' + page + '&min=' + min + '&max=' + max + '&cate=' + request + '&order_by=' + orderBy;
        $('.loader-wrapper').show();
        $.ajax({
            url: urlRedirect,
            data: { page: page, min: min, max: max, request: request, orderBy: orderBy, _token: token },
            method: 'POST',
            dataType: 'html',
            success: function(data) {
                $('.result-fillter').html(data);
                $('html, body').animate({
                    scrollTop: $(".shoppage-fullwidth").offset().top
                }, 1000);
                $('.loader-wrapper').hide();
            },
            error: function(xhr, ajaxOption, throwError) {
    ...
    $(document).on("click", '.pagination-product-cate .page-numbers li', function(e) {
        e.preventDefault();
        var current = $(this);
        var link = current.find('.item').attr('href');
        var element = new URL(link);
        var page = element.searchParams.get("page");
        var orderBy = element.searchParams.get("order_by");
        var slug = current.closest('.page-numbers').find('.prod-cate-action').data('action');
        var url = "../san-pham" + '/' + slug + '.html';
        var token = $('meta[name="csrf-token"]').attr('content');
        var urlRedirect = url + '?page=' + page + "&cate=" + slug + "&order_by=" + orderBy;
        $('.loader-wrapper').show();
        $.ajax({
            url: urlRedirect,
            method: 'POST',
            data: { page: page, _token: token, slug: slug, orderBy: orderBy },
            dataType: 'html',
            success: function(data) {
                $('.result-load-more').html(data);
                $('html, body').animate({
                    scrollTop: $(".shoppage-fullwidth").offset().top
                }, 1000);
                $('.loader-wrapper').hide();
    ...
    $(document).on("click", '.pagination-product .page-numbers li', function(e) {
        e.preventDefault();
        var current = $(this);
        var link = current.find('.item').attr('href');
        var element = new URL(link);
        var page = element.searchParams.get("page");
        var orderBy = element.searchParams.get("order_by");
        var url = "../danh-muc/san-pham";
        var token = $('meta[name="csrf-token"]').attr('content');
        var urlRedirect = url + '?page=' + page + '&order_by=' + orderBy;
        $('.loader-wrapper').show();
        $.ajax({
            url: urlRedirect,
            method: 'POST',
            data: { page: page, _token: token, orderBy: orderBy },
            dataType: 'html',
            success: function(data) {
                $('.result-load-more').html(data);
                $('html, body').animate({
    ...
    //Load more scroll page
    $('.st2-search-field').keyup(function(e) {
        var query = $(this).val();
        var parent = $(this).closest('.st2-search-form');
        ...
        if (query != '') {
            $.ajax({
                ...
                // error: function(xhr, ajxOption, throwError) {
                //     alert(xhr.status);
                //     alert(throwError);
                // }
            });
    ...
    $('.remove-content-search .fa-times').click(function(e) {
        $('.data-search').fadeOut();
    });

    // $('.data-search-content li').click(function(e) {
    //     $(this).closest('.data-search').fadeOut();
    //     // $('.data-search').fadeOut();
    // });
    ...
    function getSizeOfColor() {
        ...
        var qty = $('.product-color .color-item').first().find('.color-info').data('qty');
        $('.product-detail-buy .quanlity .quanlity-num').attr('max', qty);
